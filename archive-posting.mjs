...
export async function archiveUrl(browser, url, { company: companyHint, role: roleHint } = {}) {
  ...
  // Stealth context, not browser.newContext(): archiving meets the same
  // challenge walls as every other outbound flow, so it gets the shared UA,
  // locale, viewport, and init script from browser-launch.mjs.

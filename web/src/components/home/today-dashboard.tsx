...
export function TodayDashboard({
  ...
  const [freshCountRaw, setFreshCountRaw] = useState(0);
  ...
  const freshCount = Math.max(0, freshCountRaw - (freshRaw.length - fresh.length));
  ...
  const refetch = useCallback(() => {
    ...
    fetch("/api/whats-new")
      ...
      .then((d) => {
        ...
        setFreshRaw(offers);
        setFreshCountRaw(Number.isFinite(count) ? Math.max(0, Math.trunc(count)) : offers.length);
      })
      .catch(() => {});
  }, []);

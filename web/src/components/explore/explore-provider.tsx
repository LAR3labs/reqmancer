  ...
  const runAgentSearch = useCallback(async (opts: { endpoint: string; source: string; requireIntent: boolean; casting: string }) => {
    ...
    try {
      ...
      if (!r.ok) {
        const d = await r.json().catch(() => ({}));
        sawScannerMissing = isScannerMissing(d);
        sawError = d.error || (sawScannerMissing ? "This search isn't available." : `Search failed (${r.status}).`);
      } else if (!r.body) {
        sawError = "No response stream.";
      } else {
        const reader = r.body.getReader();
  ...
  useEffect(() => {
    ...
    try {
      const snap: ResultSnapshot = {
        ...
        partial, status, error, scannerMissing, added: [...added], dismissed: [...dismissed], aiTrace, aiCost, aiIntent,
      };
      sessionStorage.setItem(RESULTS_KEY, JSON.stringify(snap));
    } catch {
      /* sessionStorage full/unavailable — non-fatal */
    }
  }, [phase, mode, offers, matchCount, companiesScanned, companiesAvailable, capHit, droppedNoDate, sources, partial, status, error, scannerMissing, added, dismissed, aiTrace, aiCost, aiIntent]);

  const value = useMemo(
    () => ({
      filters, setFilters, initFilters, phase,
      running: phase === "casting" || phase === "scanning" || phase === "revealing" || phase === "hunting",
      offers, sources, matchCount, companiesScanned, companiesAvailable, capHit, droppedNoDate, status, partial, error, scannerMissing, added, adding, dismissed, dismissing,
      discover, loadFresh, addToPipeline, dismiss, applyPatch, reset,
      mode, setMode, aiIntent, setAiIntent, discoverAI, discoverDeep, aiTrace, aiCost,
    }),
    [filters, setFilters, initFilters, phase, offers, sources, matchCount, companiesScanned, companiesAvailable, capHit, droppedNoDate, status, partial, error, scannerMissing, added, adding, dismissed, dismissing, discover, loadFresh, addToPipeline, dismiss, applyPatch, reset, mode, setMode, aiIntent, discoverAI, discoverDeep, aiTrace, aiCost],
  );
